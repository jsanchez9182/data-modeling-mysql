...
class GoogleBooksClient:
    ...
    def get_output_path(self) -> Path:
        ...
        """
        return Path(f'raw_data/{self._keyword}/{date.today().isoformat()}/start_index_{self._start_index}.json')
    ...
    def _handle_response(self, response: requests.Response) -> None:
        ...
        if response.status_code == 200:
            ...
            logger.info(f'keyword: {self._keyword}, start_index: {self._start_index}, max_results: {self._max_results},'
...
def search_google_keywords(keywords: list[str], end_index: int,  max_results: int) -> None:
    """
    ...
    for keyword in keywords:
        client = GoogleBooksClient(keyword, 0, end_index, max_results)
        client.pull_data()
